...
namespace hallodoc_mvc_Repository.Implementation
{
    public class Admin_Repository : IAdmin_Repository
    {
        ...
        {
            return _context.AspNetUsers.Where(x => x.Roles.Where(x => x.Id == 2 || x.Id == 3).Any()).Include(x => x.AdminAspNetUsers).Include(x => x.PhysicianAspNetUsers).ThenInclude(x=>x.Requests).Include(x => x.Roles).ToList();
        }
        ...
        public List<Physician> DayDataPending()
        {
            return _context.Physicians.Include(x => x.Shifts).ThenInclude(x => x.ShiftDetails.Where(x => x.IsDeleted == new BitArray(1, false) && x.Status == 0)).ToList();
        }

        public string GetRoleOfUser(int admin)
        {
